...
export default function InvoicePrintModal({
  ...
  // Default scale to 1.5 since the original 1.0 was too tiny and left huge whitespace
  ...
  const renderContent = (isPreview: boolean) => (
    <div className={`bg-white text-black border border-black flex flex-col ${isPreview ? 'w-[210mm] h-[148mm] shadow-xl overflow-hidden' : 'w-[210mm] h-[148mm] overflow-hidden'}`} style={{ fontFamily: 'Arial, sans-serif' }}>
      ...
      <div className="flex border-b border-black" style={{ minHeight: 0 }}>
        {/* Left - Customer Info */}
        <div className="w-[30%] px-1.5 py-1 border-r border-black flex flex-col justify-center" style={{ fontSize: `${8 * scale}px`, lineHeight: '1.3' }}>
        ...
        <div className="w-[45%] px-1 py-1 text-center flex flex-col justify-center items-center">
          <h1 style={{ fontSize: `${15 * scale}px`, fontWeight: 900, letterSpacing: '0.5px', textTransform: 'uppercase', lineHeight: 1.1 }}>SHREE MAHA GANESH JEWELLERS</h1>
          <p style={{ fontSize: `${7 * scale}px`, marginTop: '2px', lineHeight: 1.2 }}>Shop No. 4 &amp; 5, Sindhu Nilayam, Sri Ram Nagar Colony, Gangaram, Chandanagar, Hyderabad-50. TG.</p>
          <p style={{ fontSize: `${8 * scale}px`, fontWeight: 700, marginTop: '2px' }}>📞 8008666199  📞 9014659444</p>
        </div>

        {/* Right - Invoice Info */}
        <div className="w-[25%] px-1.5 py-1 border-l border-black flex flex-col justify-center text-right" style={{ fontSize: `${8 * scale}px`, lineHeight: '1.3' }}>
      ...
      {/* GST INVOICE Title */}
      <div className="text-center font-bold uppercase tracking-widest border-b border-black bg-gray-50 print:bg-white" style={{ fontSize: `${8 * scale}px`, padding: '2px 0', lineHeight: 1 }}>
        GST - INVOICE
      </div>

      {/* Table */}
      <table className="w-full border-collapse" style={{ fontSize: `${7 * scale}px` }}>
        <thead className="border-b border-black bg-gray-50 print:bg-white">
          <tr>
            ...
            <th className="px-0.5 text-right" style={{ padding: '2px 4px', fontWeight: 700, fontSize: `${8 * scale}px` }}>Amount</th>
          </tr>
        </thead>
        <tbody>
          {completedInvoice.items && completedInvoice.items.map((i: any, x: number) => (
            <tr key={x} className="border-b border-black" style={{ height: `${18 * scale}px` }}>
              <td className="border-r border-black text-center" style={{ padding: '0 2px', fontSize: `${7 * scale}px` }}>{x + 1}</td>
              <td className="border-r border-black" style={{ padding: '1px 3px' }}>
                {/* Product name — MOST IMPORTANT, biggest in table */}
                <div style={{ fontSize: `${9 * scale}px`, fontWeight: 800, lineHeight: 1.1 }}>{i.category}</div>
                <div style={{ fontSize: `${6 * scale}px`, color: '#555', lineHeight: 1 }}>{i.purity} · {i.barcode}</div>
              </td>
              ...
              <td className="border-r border-black text-right" style={{ padding: '0 2px', fontSize: `${7 * scale}px` }}>{i.st_amount}</td>
              {/* Amount — MOST IMPORTANT, bold and bigger */}
              <td className="text-right" style={{ padding: '0 4px', fontSize: `${10 * scale}px`, fontWeight: 800 }}>{(i.salePrice || (i.metal_value + i.making_charge)).toFixed(2)}</td>
            </tr>
          ))}
          {completedInvoice.exchanges && completedInvoice.exchanges.map((i: any, x: number) => (
            <tr key={`exc-${x}`} className="border-b border-black" style={{ height: `${18 * scale}px`, fontStyle: 'italic', color: '#444' }}>
              <td className="border-r border-black text-center" style={{ padding: '0 2px', fontSize: `${7 * scale}px` }}>*</td>
              <td className="border-r border-black" style={{ padding: '1px 3px', fontSize: `${8 * scale}px`, fontWeight: 700 }}>Old {i.metal} Exchange</td>
              <td className="border-r border-black text-center" style={{ padding: '0 2px', fontSize: `${7 * scale}px` }}>-</td>
              <td className="border-r border-black text-center" style={{ padding: '0 2px', fontSize: `${7 * scale}px` }}>1</td>
              <td className="border-r border-black text-right" style={{ padding: '0 2px', fontSize: `${7 * scale}px` }}>{i.gross_wt}</td>
      ...
      <div className="flex border-t border-black mt-auto">
        ...
        <div className="w-[75%] border-r border-black flex flex-col">
          <div className="flex border-b border-black flex-1">
            {/* Payment info */}
            <div className="w-[50%] px-1.5 py-1 border-r border-black flex flex-col" style={{ fontSize: `${7 * scale}px`, lineHeight: '1.3' }}>
              <p><strong>SMID :</strong> {completedInvoice.sales_person_id || ''}</p>
              <div className="mt-auto">
                <p style={{ fontSize: `${8 * scale}px`, fontWeight: 800, textTransform: 'uppercase', borderBottom: '1px solid #ccc', paddingBottom: '1px', marginBottom: '2px' }}>
                ...
                {completedInvoice.payment_breakdown && (
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                    {Object.entries(
                      ...
                      <span key={k} style={{ fontWeight: 700, fontSize: `${8 * scale}px` }}>{k}: {Number(v).toFixed(2)}</span>
                    ))}
                  </div>
                )}
              </div>
            ...
            <div className="w-[50%] px-1.5 py-1" style={{ fontSize: `${6 * scale}px`, lineHeight: '1.2' }}>
          ...
          {/* Signatures */}
          <div className="flex justify-between items-end px-2 pb-1" style={{ height: `${30 * scale}px`, fontSize: `${7 * scale}px` }}>
            <div className="flex items-end">
              <p style={{ borderTop: '1px solid black', paddingTop: '2px' }}>Customer Sign</p>
            </div>
            <div className="text-center" style={{ fontSize: `${8 * scale}px`, fontWeight: 800, letterSpacing: '1px' }}>
              THANK YOU *** VISIT AGAIN
            </div>
            <div className="text-right flex flex-col justify-between items-end h-full">
              <p style={{ fontSize: `${6 * scale}px` }}>For SHREE MAHA GANESH JEWELLERS</p>
        ...
        <div className="w-[25%] flex flex-col justify-end">
          <div className="flex justify-between px-1.5 py-0.5 border-b border-black" style={{ fontSize: `${7 * scale}px` }}>
            <span>Discount :</span>
          ...
          <div className="flex justify-between items-center px-1.5 py-1" style={{ background: '#f0f0f0', borderTop: '1px solid black' }}>
            <span style={{ fontSize: `${8 * scale}px`, fontWeight: 800 }}>Net Total :</span>
            <span style={{ fontSize: `${12 * scale}px`, fontWeight: 900 }}>₹{completedInvoice.grand_total?.toFixed(2)}</span>
          </div>
        </div>
      </div>
    </div>
